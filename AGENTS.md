...
- The homepage's other sections (home/hero, about, service, testimonial) are **not** Zola content — they're driven by TOML data files under `static/sections/<section>/<lang>.toml`, loaded at render time via Tera's `load_data(..., required=false)` in each `themes/portio-zola/templates/partials/sections/*.html` partial (e.g. `{% set path = "static/sections/home/" ~ lang ~ ".toml" %}`). To change hero/about/service/testimonial copy, edit the matching `static/sections/*/en.toml`, not a template. The theme's `templates/index.html` assembles the homepage by including `home`, `about`, `service`, `resume`, `skill`, `testimonial` partials in that order; the project overrides this file at the root `templates/index.html` (see below) solely to insert a `partners_slider` include between `home` and `about` — otherwise identical. **`resume` and `skill` currently render nothing**: their data files only exist under the theme's own `themes/portio-zola/static/sections/{resume,skill}/`, and `load_data()` resolves paths against the project root, not the theme's static tree, so with `required=false` those partials silently no-op. Add real `static/sections/resume/en.toml` / `skill/en.toml` files to bring them back. Portfolio and blog are not part of this homepage assembly at all; blog is a standalone content section reached via nav.
- The project's own `templates/` directory is **not** empty — it has several overrides that back custom content types, all `{% extends "base.html" %}` (except the partial, which isn't a page template):
  ...
  - `templates/expertise.html` — used by `content/nix/` and `content/rust/`; renders `[extra]` fields `intro`/`intro_business` (Markdown, with an audience toggle button when both are set — powered by `static/js/expertise-toggle.js`), `benefits`/`services` (icon/title/content grids), `track_record`, `credential` (see partners note below), and a `cta` link to `/contact/`.
  - `templates/values.html` — used by both `content/values/` and `content/partners/`; renders a `[[extra.values]]` icon/logo/title/content grid (see partners note below for the partners-specific fields).
  - `templates/index.html` — homepage override, see above.
  - `templates/partials/sections/partners_slider.html` — homepage partial, see partners note below (not a standalone page template, included by the `index.html` override rather than by front matter).
...
**Partners — single source of truth, four consumers:** `content/partners/_index.md` (`_index.de.md` for German) is the only place partner/membership data is authored — a `[[extra.values]]` array of `{ icon, logo, logo_dark?, title, link, link_title?, link_external?, content }` entries, rendered as a grid by `templates/values.html` (a generic template also used, unfilled with partner-specific fields, by `content/values/_index.md` for the separate company-values page — hence the filename). Three other places pull from this same section instead of hardcoding logos/links:
  - **Homepage slider** — `templates/partials/sections/partners_slider.html` (included by the project's `templates/index.html` override, between `home` and `about`) loops over *all* `partners.extra.values` entries, repeated 4× for a seamless CSS-only marquee, rendering each via `macros::logo_link`.
  - **Footer** — `themes/portio-zola/templates/partials/footer.html` shows a *curated, ordered subset*: it looks up entries by exact `title` match against a hardcoded list (currently `["Nix Commercial Support", "Rust Commercial Network"]`) so only specific partners appear in a specific order, also via `macros::logo_link`. To add/remove/reorder a footer seal, edit that title list, not the partners content.
  - **Nix/Rust expertise pages** — `templates/expertise.html` renders a single credential named by that page's `extra.credential` front matter field (e.g. `content/nix/_index.md`), resolved against `partners.extra.values` by `macros::credential_pill` (CTA-area pill, links to `#partnership` on the same page) and `macros::credential_logo` (floated logo on the track-record card) in `themes/portio-zola/templates/macros.html`.
  - All four consumers go through the shared `macros::logo_link(item, link, tab)` macro (same file), which handles the `logo`/`logo_dark` theme-aware image swap and optional `link`/`link_title`/`link_external` wrapping — extend that macro rather than duplicating logo/link markup if a new consumer needs it.
